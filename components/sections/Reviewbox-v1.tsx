...
export function ReviewboxV1({
  ...
  const displayLogoSrc = preview?.settings.logoSrc ?? logoSrc;
  const displayLogoAlt = preview?.settings.logoAlt ?? logoAlt;
  const displayEyebrow = preview?.settings.eyebrow ?? eyebrow;
  const displayHeadlineLines = preview?.settings.headlineLines ?? headlineLines;
  const displaySubtext = preview?.settings.subtext ?? subtext;
  const displayProductLink = preview?.settings.productLink ?? productLink;
  const displayBullets = preview?.settings.bullets ?? bullets;
  const displayFeatures = preview?.settings.features ?? features;
  const displayFeaturesLabel = preview?.settings.featuresLabel ?? featuresLabel;
  const displayCtaLabel = preview?.settings.ctaLabel ?? ctaLabel;
  const displayCtaSubtext = preview?.settings.ctaSubtext ?? ctaSubtext;
  const displayCtaHref = preview?.settings.ctaHref ?? ctaHref;
  const displayDesktop = preview?.settings.desktop ?? desktop;
  const displayLeftExample = preview?.settings.leftExample ?? leftExample;
  const displayRightMessage = preview?.settings.rightMessage ?? rightMessage;
      ...
      <Container className="relative">
        <div className="grid items-center gap-12 lg:grid-cols-12 lg:gap-16">
          <div className="lg:col-span-5">
              ...
              <div className="mt-6 border-t border-border/60 pt-5">
                ...
                <ul className="mt-3 flex flex-wrap gap-2">
                  {displayFeatures.map((feature) => (
                    <li
                      key={feature}
                      className="reviewbox-v1-feature-pill rounded-full border border-border bg-surface/40 px-3 py-1 text-xs font-medium text-foreground sm:text-sm"
                    >
                      {feature}
                    </li>
                  ))}
            ...
            <div className="mt-8">
              <Link href={displayCtaHref} target="_blank" rel="noopener noreferrer">
                <Button
                  size="lg"
                  variant="ghost"
              ...
              {displayCtaSubtext && (
                <p className="reviewbox-v1-cta-subtext mt-2 text-center text-xs sm:text-left">{displayCtaSubtext}</p>
              )}
            </div>
          </div>
          ...
          <div className="lg:col-span-7">
            <ReviewboxShowcase
              ...
              leftExample={displayLeftExample}
              rightMessage={displayRightMessage}
            />
          </div>
        </div>
      </Container>
