...
export function HeroV21({
  ...
  const displaySubtextLines = preview?.settings.subtextLines ?? subtextLines;
  const displayHighlights = preview?.settings.highlights ?? highlights;
    ...
    >
      ...
      <Container className="relative grid items-center gap-8 lg:grid-cols-2 lg:gap-12">
        <div>
          ...
          <RotatingSubtext lines={displaySubtextLines} />
          {displayCtaLabel && displayCtaHref && (
            <div className="mt-6">
              <a
                href={displayCtaHref}
                aria-label={`${displayCtaLabel} — contact ${siteConfig.name}`}
                onClick={(event) => {
                  if (modal && isContactHref(displayCtaHref)) {
                    event.preventDefault();
                    modal.openContact();
                  }
                }}
              >
                <PreviewButton>{displayCtaLabel}</PreviewButton>
              </a>
            </div>
          )}
        </div>
