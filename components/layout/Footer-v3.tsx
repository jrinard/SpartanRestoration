...
import { getRuntimePolicies } from "@/lib/org/runtime-policies";
...
function formatFooterServiceAreas(areas: readonly string[]): string {
  const cities = areas.map((area) => area.replace(/, WA$/, ""));
  if (cities.length === 0) return siteConfig.serviceArea;
  return `${cities.join(", ")} & Clark County`;
}
...
export function FooterV3({ description }: FooterV3Props) {
  ...
  const footerServiceAreaText = siteConfig.serviceAreas?.length
    ? formatFooterServiceAreas(siteConfig.serviceAreas)
        ...
        >
          <div className="flex flex-col gap-12 lg:flex-row lg:items-start lg:justify-between">
            ...
            <div className="flex flex-col gap-3 sm:items-end">
              ...
              {footerServiceAreaText && (
                <p className="footer-v3-service-area max-w-sm text-sm leading-relaxed sm:text-base sm:text-right">
                  {footerServiceAreaText}
                </p>
              )}
              {siteConfig.phone && (
              ...
              <div className="footer-v3-legal-links mt-1 flex flex-col gap-1 sm:items-end">
                {isOrgPolicyPublished(getRuntimePolicies().privacy) ? (
                  <Link
                    href="/privacy"
                    className="footer-v3-contact-link text-sm transition-colors"
                  >
                    Privacy Policy
                  </Link>
                ) : null}
                {isOrgPolicyPublished(getRuntimePolicies().terms) ? (
                  <Link
                    href="/terms"
                    className="footer-v3-contact-link text-sm transition-colors"
                  >
