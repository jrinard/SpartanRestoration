...
import {
  ...
  getTextImageImageBoxShadow,
  textImageImageShadowMax,
  ...
  return (
    <div className="contents">
      ...
      <label className="flex min-w-[10rem] flex-1 items-center gap-2">
        <span className="shrink-0 font-mono text-xs tracking-wide text-accent-purple uppercase">
          Img Shadow
        </span>
        <input
          type="range"
          min={0}
          max={textImageImageShadowMax}
          step={1}
          value={context.settings.imageShadowIntensity}
          onChange={(event) => update({ imageShadowIntensity: Number(event.target.value) })}
          className="h-1.5 min-w-0 flex-1 accent-accent-purple"
          aria-label="Text and image photo drop shadow intensity"
          aria-valuemin={0}
          aria-valuemax={textImageImageShadowMax}
          aria-valuenow={context.settings.imageShadowIntensity}
        />
        <span className="w-7 shrink-0 font-mono text-[0.65rem] text-accent-purple/80 tabular-nums">
          {context.settings.imageShadowIntensity}
        </span>
      </label>
      <label className="flex items-center gap-2">
