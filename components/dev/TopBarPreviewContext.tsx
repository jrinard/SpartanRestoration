  ...
  return (
    <>
      ...
      <label className="flex items-center gap-2">
        <input
          type="checkbox"
          checked={context.settings.inverted === true}
          onChange={(event) => update({ inverted: event.target.checked })}
          className="h-3.5 w-3.5 accent-accent-purple"
          aria-label="Invert top bar sides"
        />
        <span className="font-mono text-xs tracking-wide text-accent-purple uppercase">Inverted</span>
      </label>
      <label className="flex items-center gap-2">
