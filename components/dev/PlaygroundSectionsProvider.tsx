...
import {
  ...
  setPlaygroundPageIncludeInNav,
  setPlaygroundPageRoute,
  upsertPlaygroundLocationPagesFromConfig,
...
import { orgStorageGet } from "@/lib/org/browser-storage";
import { seedSectionCopyIfMissing } from "@/lib/seed-section-copy";
import { seedServiceAreaLocationHeroes } from "@/lib/service-area-location-hero-seed";
import { autosaveHomepageConfigFromStorage, collectHomepageConfigFromStorage } from "@/lib/homepage-export-client";
import { useOptionalOrg } from "@/components/dev/OrgProvider";
...
type PlaygroundSectionsContextValue = {
  ...
  setPageRoute: (pageId: string, route: string) => string | null;
  setIncludeInNav: (pageId: string, includeInNav: boolean) => void;
  ...
  policiesEditorOpen: boolean;
  setPoliciesEditorOpen: (open: boolean) => void;
  seoEditorOpen: boolean;
  setSeoEditorOpen: (open: boolean) => void;
  ready: boolean;
};
...
export function PlaygroundSectionsProvider({ children }: { children: ReactNode }) {
  ...
  const [policiesEditorOpen, setPoliciesEditorOpen] = useState(false);
  ...
  useEffect(() => {
    ...
    let state = loadPlaygroundPagesState();
    const workshopConfig = org.pillar.workshopConfig;
    if (workshopConfig) {
      const synced = upsertPlaygroundLocationPagesFromConfig(state, workshopConfig);
      if (synced !== state) {
        savePlaygroundPagesState(synced);
        state = synced;
      }
    }
    seedServiceAreaLocationHeroes(state, workshopConfig);
    skipNextSaveRef.current = true;
    setPagesState(state);
    setReady(true);
  }, [org?.orgId, org?.pillar?.workshopConfig]);

  useEffect(() => {
    if (!ready) return;

    // Avoid persisting the pre-hydration default layout over saved localStorage.
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      if (!dirtyRef.current) return;
      const config = JSON.stringify(collectHomepageConfigFromStorage());
      if (config === lastAutosaveRef.current) {
        dirtyRef.current = false;
        return;
      }
      void autosaveHomepageConfigFromStorage()
        .then(() => {
          lastAutosaveRef.current = config;
          dirtyRef.current = false;
        })
        .catch(() => {
          /* keep dirty so close-tab still warns */
        });
    }, 8000);
  ...
  const setPageRoute = useCallback((pageId: string, route: string) => {
    let error: string | null = null;
    setPagesState((current) => {
      const result = setPlaygroundPageRoute(current, pageId, route);
      if (!result.ok) {
        error = result.error;
        return current;
      }
  ...
  const setIncludeInNav = useCallback((pageId: string, includeInNav: boolean) => {
    setPagesState((current) => setPlaygroundPageIncludeInNav(current, pageId, includeInNav) ?? current);
  }, []);
  ...
  const updateSection = useCallback((id: string, patch: Partial<PlaygroundSectionConfig>) => {
    setPagesState((current) => {
      ...
      const nextSections = pageSections.map((section) => {
        ...
        const next = applyPlaygroundSectionPatch(section, patch);
        if (patch.hidden === false || patch.preview === true) {
          seedSectionCopyIfMissing(next);
        }
        return next;
      });
