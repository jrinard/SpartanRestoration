...
import {
  ...
  navBarHeightOptions,
  navBarLayoutWidths,
  navBarMobileToggleAlignOptions,
...
export function NavBarPreviewControls() {
  ...
  return (
    <div className="contents">
      ...
      <label className="flex items-center gap-2">
        <span className="font-mono text-xs tracking-wide text-accent-purple uppercase">Mobile Menu</span>
        <select
          value={context.settings.mobileToggleAlign}
          onChange={(event) =>
            update({ mobileToggleAlign: event.target.value as NavBarMobileToggleAlign })
          }
          className={selectClassName}
          aria-label="Nav bar mobile menu button alignment"
        >
          {navBarMobileToggleAlignOptions.map((option) => (
            <option key={option.value} value={option.value}>
              {option.label}
            </option>
          ))}
