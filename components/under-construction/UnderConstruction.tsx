...
import { assetExists, getAssetUrl } from "@/lib/assets.server";
...
export function UnderConstruction() {
  const logoPath = siteConfig.underConstruction.logo ?? siteConfig.assets.logo;
  const hasLogo = assetExists(logoPath);
  ...
  return (
    <div
      className="under-construction relative flex min-h-screen flex-col overflow-hidden"
      data-uc-theme={siteConfig.assets.themeFolder}
    >
              ...
              {hasLogo ? (
                <div className="flex items-center justify-center">
                  <Image
                    ...
                    width={1137}
                    height={352}
                    className="h-auto w-full max-w-[min(90vw,28rem)] object-contain sm:max-w-[34rem]"
                    priority
                  />
                </div>
              ) : (
