...
export type TextImagePreviewSettings = TextImageTextColors &
  ButtonPreviewSettings & {
    ...
    /** Image drop shadow intensity (0 = none, 100 = strongest). */
    imageShadowIntensity: number;
...
export const textImageImageShadowMax = 100;

/** Down-and-right box shadow for the content image; undefined when intensity is 0. */
export function getTextImageImageBoxShadow(intensity: number): string | undefined {
  const value = Math.min(
    textImageImageShadowMax,
    Math.max(0, Math.round(intensity)),
  );
  if (value === 0) return undefined;

  const t = value / textImageImageShadowMax;
  const offsetX = Math.round(2 + t * 14);
  const offsetY = Math.round(3 + t * 16);
  const blur = Math.round(6 + t * 28);
  const alpha = 0.06 + t * 0.28;

  return `${offsetX}px ${offsetY}px ${blur}px rgba(0, 0, 0, ${alpha.toFixed(2)})`;
}
