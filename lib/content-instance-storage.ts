...
export type ContentInstanceSettings = Pick<
  ...
  "textIconsV3" | "textImage" | "textImages" | "imagesV1"
>;
...
export function loadContentInstanceSettings(
  ...
  const imagesV1 = loadSectionInstanceField(instanceId, "imagesV1");

  if (!textIconsV3 && !textImage && !textImages && !imagesV1) return undefined;

  return { textIconsV3, textImage, textImages, imagesV1 };
}
...
export function loadImagesV1InstanceSettings(
  instanceId: string,
): ImagesV1PreviewSettings | undefined {
  const stored = loadSectionInstanceField(instanceId, "imagesV1");
  return stored ? normalizeImagesV1PreviewSettings(stored) : undefined;
}

export function saveImagesV1InstanceSettings(
  instanceId: string,
  settings: ImagesV1PreviewSettings,
): void {
  patchSectionInstanceSettings(instanceId, {
    imagesV1: normalizeImagesV1PreviewSettings(settings),
  });
}

export function loadEffectiveImagesV1InstanceSettings(
  instanceId: string,
): ImagesV1PreviewSettings {
  return loadImagesV1InstanceSettings(instanceId) ?? loadImagesV1PreviewSettings();
}
...
export function copyEffectiveContentInstanceSettings(
  ...
  if (variant === "images-v1") {
    patchSectionInstanceSettings(toId, {
      ...current,
      imagesV1: structuredClone(loadEffectiveImagesV1InstanceSettings(fromId)),
    });
    return;
  }
