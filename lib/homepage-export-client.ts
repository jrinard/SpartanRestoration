...
import { footerV4PreviewStorageKey } from "@/lib/footer-v4-preview-storage";
...
import { getClientOrgId, orgStorageGet } from "@/lib/org/browser-storage";
import {
  normalizeHomepageConfigForSave,
  reconcileHomePageGlobalPreviewSettings,
} from "@/lib/home-preview-sync";
import { applyHomepageConfigToStorage } from "@/lib/homepage-import-client";

function readJson<T>(key: string): T | undefined {
  try {
    const raw = orgStorageGet(key);
...
function collectPreviewSettingsFromPages(
  ...
  for (const page of pagesState.pages) {
    ...
    for (const section of previewSectionSlots) {
      ...
      if (settings.textIconsV3 || settings.textImage || settings.textImages || settings.imagesV1) {
        contents[section.id] = {
          textIconsV3: settings.textIconsV3,
          textImage: settings.textImage,
          textImages: settings.textImages,
          imagesV1: settings.imagesV1,
        };
      }
    }
  }
  ...
  const previewSettings: HomepagePreviewSettings = {
    ...
    heroV4: readJson(heroV4PreviewStorageKey),
    ...
    footerV4: readJson(footerV4PreviewStorageKey),
    portfolio: readJson(portfolioPreviewStorageKey),
    portfolioV2: readJson(portfolioV2PreviewStorageKey),
    ...
    imagesV1: readJson(imagesV1PreviewStorageKey),
    servicesIconsV2: readJson(servicesIconsV2PreviewStorageKey),
    serviceAreaV2: readJson(serviceAreaV2PreviewStorageKey),
    reviews: readJson(reviewsPreviewStorageKey),
    analytics: (() => {
...
async function postHomepageConfigAction(
  action: "publish" | "stage" | "autosave",
  config: HomepageConfig,
): Promise<void> {
  const response = await fetch("/api/homepage-config", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action, config, orgId: getClientOrgId() }),
  });
  ...
  if (!response.ok) {
    ...
    const fallback =
      action === "autosave"
        ? "Failed to autosave canvas."
        : action === "stage"
          ? "Failed to save staging preview."
          : "Failed to publish homepage.";
    throw new Error(message || fallback);
  }
}

export async function autosaveHomepageConfigFromStorage(): Promise<void> {
  const config = collectHomepageConfigFromStorage();
  await postHomepageConfigAction("autosave", config);
}
...
export async function stageHomepageConfigFromStorage(): Promise<void> {
  ...
  applyHomepageConfigToStorage(config);
}
...
export async function revertHomepageToConstruction(): Promise<void> {
  const response = await fetch("/api/homepage-config", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "revert", orgId: getClientOrgId() }),
  });
