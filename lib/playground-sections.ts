...
import {
  createPlaygroundSectionId,
  getDefaultPlaygroundSectionId,
} from "@/lib/playground-section-id";
...
const defaultSectionDefs: Omit<PlaygroundSectionConfig, "id">[] = [
  ...
  { group: "reviews", defaultVariant: "reviews-v1", preview: false },
...
export const defaultPlaygroundSections: PlaygroundSectionConfig[] = defaultSectionDefs.map(
  (section) => ({
    ...section,
    id: getDefaultPlaygroundSectionId(section.group),
  }),
);
...
export function applyPlaygroundSectionPatch(
  ...
  if (patch.hidden === false && canIncludePlaygroundSectionInPreview(next.group)) {
    next.preview = true;
  }
...
function insertIndexForMissingSection(
  sections: PlaygroundSectionConfig[],
  group: SectionGroupId,
): number {
  if (group === "topBar") return 0;

  if (group === "nav") {
    const headerIndex = sections.findIndex((section) => section.group === "header");
    return headerIndex === -1 ? sections.length : headerIndex + 1;
  }

  const defaultIndex = defaultPlaygroundSections.findIndex((section) => section.group === group);
  if (defaultIndex === -1) return sections.length;

  for (let i = defaultIndex - 1; i >= 0; i -= 1) {
    const anchorGroup = defaultPlaygroundSections[i]!.group;
    for (let j = sections.length - 1; j >= 0; j -= 1) {
      if (sections[j]!.group === anchorGroup) {
        return j + 1;
      }
    }
  }
...
function insertMissingPlaygroundSection(
  sections: PlaygroundSectionConfig[],
  missingSection: PlaygroundSectionConfig,
): void {
  const insertAt = insertIndexForMissingSection(sections, missingSection.group);
  sections.splice(insertAt, 0, missingSection);
}
...
export function parsePlaygroundSectionOrder(
  ...
  for (const fallback of defaultPlaygroundSections) {
    ...
    if (!seenNonSpacerGroups.has(fallback.group)) {
      /** New registry sections should not clutter an in-progress playground layout. */
      const missingSection: PlaygroundSectionConfig = {
        ...fallback,
        preview: false,
        hidden: true,
      };

      insertMissingPlaygroundSection(merged, missingSection);
  ...
  for (const fallback of defaultPlaygroundSections) {
    if (!canDuplicatePlaygroundSection(fallback.group)) continue;
    if (merged.some((section) => section.group === fallback.group)) continue;

    insertMissingPlaygroundSection(merged, {
      ...fallback,
      id: createPlaygroundSectionId(fallback.group),
      preview: false,
      hidden: true,
    });
  }
...
export function duplicatePlaygroundSection(
  ...
  const copy: PlaygroundSectionConfig = {
    ...
    id: newId,
    preview:
      source.hidden !== true && canIncludePlaygroundSectionInPreview(source.group)
        ? true
        : source.preview,
  };
