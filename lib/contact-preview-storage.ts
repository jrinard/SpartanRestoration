...
import { normalizeContactSmsOptInSettings } from "@/lib/contact-sms-opt-in";
import {
  ...
  return {
    ...
    leadToEmail:
      typeof value.leadToEmail === "string" && value.leadToEmail.trim()
        ...
        : undefined,
    smsOptIn: value.smsOptIn ? normalizeContactSmsOptInSettings(value.smsOptIn) : undefined,
  };
}
...
export function loadContactPreviewSettings(): ContactPreviewSettings {
  ...
  try {
    const stored = orgStorageGet(contactPreviewStorageKey);
    if (stored) {
...
export function saveContactPreviewSettings(settings: ContactPreviewSettings): void {
  ...
  orgStorageSet(contactPreviewStorageKey, JSON.stringify(settings));
}
