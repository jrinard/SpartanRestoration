...
export function normalizeTextImagePreviewSettings(
  ...
  return {
    ...
    imageShadowIntensity:
      typeof value.imageShadowIntensity === "number"
        ? Math.min(textImageImageShadowMax, Math.max(0, Math.round(value.imageShadowIntensity)))
        : defaultTextImagePreviewSettings.imageShadowIntensity,
...
export function loadTextImagePreviewSettings(): TextImagePreviewSettings {
  ...
  try {
    const stored = orgStorageGet(textImagePreviewStorageKey);
...
export function saveTextImagePreviewSettings(settings: TextImagePreviewSettings): void {
  ...
  orgStorageSet(textImagePreviewStorageKey, JSON.stringify(settings));
}
