...
/** Duplicate a section stack for a new playground page — fresh instance ids, same variants. */
export function cloneHomepageSections(
  sections: HomepageSectionEntry[],
...
/** Swap home `sections` for an inner page from `pages[]`. */
export function withHomepagePageSections(
  config: HomepageConfig,
  slug: string,
): HomepageConfig | null {
  const page = getHomepagePageSnapshot(config, slug);
...
function normalizeHomepagePageSnapshots(value: unknown): HomepagePageSnapshot[] | undefined {
  ...
  const pages = value.flatMap((entry) => {
    ...
    return [{ slug: entry.slug, name: entry.name, sections, includeInNav: entry.includeInNav }];
  });
